...
| `CODERLEGION_API_KEY` | cross-posting the same collection to CoderLegion (`edicola.mjs`) |
...
doppler run -- node engine/edicola.mjs                                 # Newsstand cards + CoderLegion cross-post of the articles published on dev.to
doppler run -- node engine/advance.mjs                                 # decides which magazine stage to run (prints, nothing else)
gh api "<...>/check-runs" | node engine/edicola-gate.mjs <state> <tries>  # decides whether the card PR can be merged (prints, nothing else)
...
- `edicola.mjs`: automatic Newsstand cards, **and** the CoderLegion cross-post. It queries dev.to (**published** articles whose canonical points at the site) and adds the missing cards to `src/data/edicola.json` (pure merge in `lib/edicola.mjs`, deduped by slug and by url; the label comes from the `edicola` frontmatter field or from the title). Every piece entering the stack for the first time is also created on CoderLegion (`lib/coderlegion.mjs`), and its post id is kept in the card's `coderlegion` field — which is the whole ledger: CoderLegion never returns `source_url` and reserves "my posts" to the Master Key, so nothing over there can be asked "have I already published this?". Cards already in the stack are never touched, so the first run does not republish the archive. Declared ceiling: if a run dies between the create and the merge, the next day makes a duplicate — hide it with `POST /posts/{id}/hide`. The `edicola-card.yml` cron opens the PR and carries it to production once the gates are green.
...
- `edicola-gate.mjs`: the decision-maker of the card's automatic merge. It reads the `check-runs` response on stdin and prints `mergia` | `aspetta` | `ferma`; the reason goes to stderr. The pure decision lives in `lib/gate-merge.mjs` (tested dry). Green required checks and "the PR is mergeable" are two different questions: GitHub recomputes mergeability asynchronously, so a merge fired the second the last check turns green still hits `BLOCKED`. The gate waits for both.
