...
`@sentry/cloudflare` reads `SENTRY_ENVIRONMENT` off the Worker's `env` by itself, and without it
the SDK falls back to `production`: a crash from your own laptop lands in the same pile as a
visitor's, and 14% of one month's volume arrived that way. Sentry's inbound filter drops what
comes from `127.0.0.1`, which is not the same thing — an event carrying the
